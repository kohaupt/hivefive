...
import numpy as np
from hive_dataset_kaggle import HiveDatasetKaggle
import os
import pandas as pd
...
class TrainKaggle():
    ...
    def __init__(self, model=None, loss_fn=None):
        # Load metadata
        metadata_column_names = ['device', 'hive number', 'date', 'hive temp', 'hive humidity','hive pressure', 'weather temp', 'weather humidity', 'weather pressure', 'wind speed', 'gust speed', 'weatherID', 'cloud coverage', 'rain', 'lat', 'long', 'file name', 'queen presence', 'queen acceptance', 'frames', 'target', 'time', 'queen status']
        ...
        metadata_df['file name'] = metadata_df['file name'].apply(lambda x: x.replace('.wav', ''))
        test_dataset = HiveDatasetKaggle(metadata_df=metadata_df, processed_data_path=config.NORMALIZED_MEL_SPEC_PATH, target_feature='queen presence')
